...
def valid_yaml_payload() -> dict[str, object]:
    return {
        ...
        "ground_station": {
            "id": "GS-DEMO-BEIJING", "simulated": True,
            "latitude": 39.9042, "longitude": 116.4074, "altitude_m": 50,
        },
        "links": {
        ...
        "sensor": {
            "bit_depth": 12, "gain": 1, "offset_dn": 32, "dark_current_dn": 4,
            "read_noise_dn": 0, "prnu_sigma": 0, "bad_pixel_rate": 0,
        },
    }
...
def test_yaml_tle_and_scenario_snapshot_are_frozen() -> None:
    ...
    scenario = ScenarioConfig(
        id="scenario-imported", scene_id="scene-imported", scene_ready=False,
        satellite_name=imported.satellite.name, tle_line1=imported.satellite.tle_line1,
        tle_line2=imported.satellite.tle_line2, links=profiles, sensor=imported.sensor,
    )
    command = MissionCommand(run_id="run-imported", scenario_id=scenario.id, scenario_snapshot=scenario)
