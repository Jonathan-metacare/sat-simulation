...
export function DesktopSettingsPanel({ open, onClose, locale, onLocale, onTheme, onAiMode, initialSection = "general", onScenarioImported }: { open: boolean; onClose(): void; locale: Locale; onLocale(value: Locale): void; onTheme(value: "dark" | "light"): void; onAiMode(value: "yolo" | "llm"): void; initialSection?: SettingsSection; onScenarioImported?(): void }) {
  ...
  const save = async () => {
    ...
    try { const saved = await bridge.saveSettings(value); setValue(saved); onLocale(saved.locale); onTheme(saved.theme); onAiMode(saved.activeAiMode); setDiagnostics(await bridge.diagnostics()); }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm">
    <section className="panel max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl p-5 shadow-2xl shadow-black/60">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div><h2 className="flex items-center gap-2 text-lg font-medium text-cyan-100"><Settings2 size={18} />{zh ? "设置" : "Settings"}</h2><p className="mt-1 text-xs leading-5 text-slate-500">{zh ? "本机配置仅保存在此设备。" : "Local configuration is stored on this device only."}</p></div>
      ...
      <div className="mb-5 flex flex-wrap gap-2">{(["general", "plugins", "ai", "scene", "about"] as const).map((item) => <button key={item} onClick={() => setSection(item)} className={`rounded-lg border px-3 py-2 text-xs ${section === item ? "border-cyan-300/50 bg-cyan-300/10 text-cyan-100" : "border-white/10 text-slate-400"}`}>{({ general: zh ? "通用" : "General", plugins: zh ? "插件与密钥" : "Plugins & Keys", ai: "AI", scene: zh ? "场景导入" : "Scene Import", about: zh ? "关于" : "About" })[item]}</button>)}</div>
      ...
      {section === "plugins" && <div className="space-y-3"><Field label="Cesium Ion Token" type="password" value={value.cesiumIonToken} onChange={(next) => update("cesiumIonToken", next)} /><p className="text-xs leading-5 text-slate-500">{zh ? "此处用于管理地图、影像及后续插件的访问密钥。留空时使用确定性仿真地球。" : "Manage map, imagery, and future plugin credentials here. Leave empty for the deterministic simulation globe."}</p></div>}
      ...
      {section === "ai" && <div className="grid gap-4 sm:grid-cols-2">
      ...
      {section === "about" && <div className="rounded-xl border border-white/[.07] bg-black/20 p-4 text-sm text-slate-300"><div className="text-cyan-200">{zh ? "软件版本" : "Software version"}</div><div className="mt-3 font-mono text-lg text-cyan-100">SIL ONLINE · v{diagnostics?.version ?? "0.1.1"}</div></div>}
