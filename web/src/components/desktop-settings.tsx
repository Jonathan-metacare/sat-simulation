...
export function DesktopSettingsPanel({ open, onClose, locale, onLocale, onTheme, onAiMode, initialSection = "general", onScenarioImported, onSettingsSaved, activeScenarioId, onScenarioSelected }: { open: boolean; onClose(): void; locale: Locale; onLocale(value: Locale): void; onTheme(value: "dark" | "light"): void; onAiMode(value: "yolo" | "llm"): void; initialSection?: SettingsSection; onScenarioImported?(): void; onSettingsSaved?(): void; activeScenarioId?: string; onScenarioSelected?(scenario: ScenarioRecord): Promise<void> | void }) {
  ...
  const importYaml = async () => {
    ...
    try { const result = await api.importScenarioYaml(yamlFile); const records = await api.scenarios(); setScenarios(records); onScenarioImported?.(); setSceneNotice(`${zh ? "YAML 校验成功，场景已创建：" : "YAML validated and scenario created: "}${result.config.id}`); }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
    finally { setWorking(false); }
  };
  return <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm">
    <section className="panel max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl p-5 shadow-2xl shadow-black/60">
      ...
      {section === "scene" && <div className="space-y-4"><p className="text-xs leading-5 text-slate-500">{zh ? "默认北京场景已经内置。这里可导入 YAML 并切换后续任务使用的场景；光学影像请在 Optical 页的 Optical Scene Input 中导入。" : "The Beijing demo is built in. Import YAML and select the scenario for subsequent missions here; import optical imagery from Optical Scene Input on the Optical page."}</p><label className="block text-xs text-slate-400">{zh ? "当前场景" : "Active scenario"}<select value={value.activeScenarioId} disabled={working} onChange={(event) => { const selected = scenarios.find((item) => item.config.id === event.target.value); if (!selected) return; setWorking(true); setError(undefined); void Promise.resolve(onScenarioSelected?.(selected)).then(() => update("activeScenarioId", selected.config.id)).catch((cause) => setError(cause instanceof Error ? cause.message : String(cause))).finally(() => setWorking(false)); }} className="mt-1.5 w-full rounded-lg border border-white/10 bg-black/25 px-3 py-2 text-sm text-slate-100">{scenarios.map((item) => <option key={item.config.id} value={item.config.id}>{item.config.name}{item.config.scene_ready ? "" : ` (${zh ? "需要在 Optical 导入影像" : "needs optical image"})`}</option>)}</select></label><div className="border-t border-white/[.07] pt-4"><label className="block text-xs text-slate-400">YAML<input className="mt-1.5 block w-full text-xs" type="file" accept=".yaml,.yml" onChange={(event) => setYamlFile(event.target.files?.[0])} /></label><button disabled={!yamlFile || working} onClick={() => void importYaml()} className="mt-2 rounded-lg border border-cyan-300/50 px-3 py-2 text-xs text-cyan-100">{zh ? "校验并导入 YAML" : "Validate and import YAML"}</button></div>{sceneNotice && <p className="text-xs text-emerald-300">{sceneNotice}</p>}</div>}
