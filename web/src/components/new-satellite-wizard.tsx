...
const noradInputClass = "w-full rounded-lg border border-white/10 bg-black/25 px-2.5 py-1.5 text-xs text-slate-100 outline-none focus:border-cyan-300/45";
...
export function NewSatelliteWizard({ locale, onClose, onCompleted }: Props) {
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="new-satellite-title">
    <section className="panel w-full max-w-md rounded-2xl p-5 shadow-2xl shadow-cyan-950/50">
      ...
      {step === 0 && <div className="mt-5 space-y-4"><div className="rounded-xl border border-cyan-300/20 bg-cyan-300/[.04] p-3"><label className="block text-xs text-slate-400">{t("satellite.norad")}</label><div className="mt-1.5 flex gap-2"><input value={noradId} inputMode="numeric" onChange={(event) => setNoradId(event.target.value.replace(/\D/g, ""))} placeholder={t("satellite.noradPlaceholder")} className={noradInputClass} /><button disabled={lookingUp || !validNorad} onClick={() => void lookupNorad()} className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-cyan-300/50 bg-cyan-300/15 px-2.5 py-1.5 text-[11px] text-cyan-100 disabled:opacity-40">{lookingUp && <LoaderCircle size={12} className="animate-spin" />}{lookingUp ? t("satellite.noradLookingUp") : t("satellite.noradLookup")}</button></div><p className={`mt-2 text-[11px] leading-4 ${lookupNotice ? "text-emerald-300" : "text-slate-500"}`}>{lookupNotice ?? t("satellite.noradHint")}</p></div><Field label={t("satellite.name")} value={satelliteName} onChange={setSatelliteName} /><Field label={t("satellite.tleLine1")} value={tleLine1} onChange={setTleLine1} /><Field label={t("satellite.tleLine2")} value={tleLine2} onChange={setTleLine2} /></div>}
