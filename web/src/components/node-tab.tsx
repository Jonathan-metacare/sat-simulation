...
function ProcessorConfiguration({ stage, scenario, locale, onChanged, locked = false, frozenProcessorId }: { stage: ProcessorStage; scenario: ScenarioConfig; locale: Locale; onChanged?: () => void; locked?: boolean; frozenProcessorId?: string }) {
  ...
  return <section className="panel rounded-2xl p-4">
    ...
    {source && <div className="rounded-xl border border-emerald-300/35 bg-emerald-300/[.06] p-3"><div className="mb-3 flex flex-wrap items-center gap-2"><span className="text-xs text-cyan-100">{translate(locale, "node.processorSource")}</span><button onClick={() => setSourceFile("python")} className={`rounded px-2 py-1 text-[10px] ${sourceFile === "python" ? "bg-cyan-300/15 text-cyan-100" : "text-slate-500"}`}>{translate(locale, "node.pythonSource")}</button><button onClick={() => setSourceFile("yaml")} className={`rounded px-2 py-1 text-[10px] ${sourceFile === "yaml" ? "bg-cyan-300/15 text-cyan-100" : "text-slate-500"}`}>{translate(locale, "node.manifest")}</button><span className="ml-auto font-mono text-[10px] text-slate-500">{workspaceMode ? translate(locale, "node.createCustom") : selected}</span></div>{sourceFile === "yaml" ? <pre style={{ height: "28rem", minHeight: "28rem" }} className="overflow-auto rounded-lg bg-black/20 p-3 font-mono text-[11px] leading-5 text-slate-300">{source.processor_yaml}</pre> : <textarea value={draft} readOnly={locked || (source.readonly && !workspaceMode)} onChange={(event) => setDraft(event.target.value)} spellCheck={false} style={{ height: "28rem", minHeight: "28rem" }} className="w-full resize-y rounded-lg border border-white/10 bg-black/25 p-3 font-mono text-[11px] leading-5 text-slate-200 outline-none focus:border-cyan-300/40 read-only:opacity-75" />}{source.readonly && !workspaceMode && <p className="mt-2 text-[10px] text-slate-500">{translate(locale, "node.sourceReadonly")}</p>}</div>}
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2"><a href={processorDownloadURL(selected)} className="rounded-lg border border-white/10 px-3 py-1.5 text-xs text-slate-300">{translate(locale, "node.download")}</a><input id={`processor-bundle-${stage}`} disabled={locked} type="file" accept=".zip,application/zip" onChange={(event) => setBundle(event.target.files?.[0])} className="sr-only" /><label htmlFor={`processor-bundle-${stage}`} className={`rounded-lg border border-emerald-300/30 bg-emerald-300/[.08] px-3 py-1.5 text-xs text-emerald-200 ${locked ? "cursor-not-allowed opacity-40" : "cursor-pointer"}`}>{translate(locale, "node.chooseZip")}</label>{bundle && <><span className="max-w-52 truncate rounded-md border border-cyan-300/35 bg-sky-100 px-2 py-1.5 font-mono text-[10px] font-medium text-slate-800 dark:bg-slate-900/80 dark:text-slate-100" title={bundle.name}>{bundle.name}</span><button disabled={locked || busy} onClick={() => void upload()} className="rounded-lg border border-emerald-300/30 bg-emerald-300/[.08] px-3 py-1.5 text-xs text-emerald-200 disabled:opacity-40">{translate(locale, "node.validateImportActivate")}</button></>}{(workspaceMode || !source?.readonly) && <button disabled={locked || busy || !draft.trim()} onClick={() => void saveWorkspace()} className="rounded-lg border border-cyan-300/35 bg-cyan-300/[.08] px-3 py-1.5 text-xs text-cyan-100 disabled:opacity-40">{translate(locale, "node.saveActivate")}</button>}</div>
    {status && <div className="mt-3 rounded-lg border border-cyan-300/15 bg-black/15 px-3 py-2 text-xs text-slate-300">{status}</div>}
  </section>;
}
