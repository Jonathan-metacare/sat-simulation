...
export interface ScenarioConfig { id:string; name:string; seed:number; epoch:string; clock_rate:1|10|100; tle_line1:string; tle_line2:string; satellite_name:string; ground_station_name:string; ground_station_latitude:number; ground_station_longitude:number; ground_station_altitude_m:number; deterministic_contact:boolean; scene_id:string; }
...
export interface OrbitSample { sampled_at:string; latitude:number; longitude:number; altitude_km:number; elevation_deg:number; visible:boolean; }
export interface ContactWindow { aos:string; los:string; max_elevation_at:string; max_elevation_deg:number; }
export interface OrbitTrack { generated_at:string; satellite_name:string; ground_station_name:string; minimum_elevation_deg:number; visibility_radius_m:number; contact_mode:"geometric"|"deterministic"; current:OrbitSample; history:OrbitSample[]; forecast:OrbitSample[]; contact_windows:ContactWindow[]; }
